...
vector<pair<int, int>> generate_connected_graph(int n, int m) {
	...
	for(int i = 0; i < (int) edges.size(); ++i) {
		used[make_pair(edges[i].first, edges[i].second)] = used[make_pair(edges[i].second, edges[i].first)] = true;
	}
...
int main() {
	...
	int n = rand_int(1, 10), m = rand_int(1, 1);
	cout << n << ' ' << m << '\n';
	...
	for(int i = 0; i < m; ++i) {
		int u = rand_int(1, n), v = rand_int(1, n), cost = rand_int(1, 1e9);
		cout << u << ' ' << v << ' ' << cost << '\n';
	}
